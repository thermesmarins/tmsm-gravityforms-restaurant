...
class Tmsm_Gravityforms_Restaurant_List_Table extends WP_List_Table {
	...
	function __construct(){
		...
		parent::__construct( array(
			'singular'  => 'restaurant-closed',     //singular name of the listed records
			'plural'    => 'restaurant-closed',    //plural name of the listed records
	...
	function get_columns(){
		$columns = array(
			...
			'title'     => __('Date', 'tmsm-gravityforms-restaurant'),
			'content'    => __('Hours Slots', 'tmsm-gravityforms-restaurant'),
		);
		return $columns;
	}
	...
	function get_bulk_actions() {
		$actions = array(
			'delete'    =>  __('Delete', 'tmsm-gravityforms-restaurant'),
		);
		return $actions;
	}
		...
		if ( isset($_GET['_wpnonce']) && wp_verify_nonce( $_GET['_wpnonce'], 'bulk-' . $this->_args['plural'] ) && 'delete' === $this->current_action() ) {
			...
			if ( is_array( $restaurant_closed_ids ) ) {
				foreach ( $restaurant_closed_ids as $restaurant_closed_id ) {
					$restaurant_closed_id = absint( wp_unslash( $restaurant_closed_id ) );
					wp_delete_post( $restaurant_closed_id );
				}

				echo self::bulk_admin_notices();
			}
		}

	}

	static function bulk_admin_notices(){
		$html =	'<div class="notice notice-success is-dismissible"><p><strong>'.__('Items were deleted successfully', 'tmsm-gravityforms-restaurant').'</strong></p></div>';
		echo $html;
	}
